...
def def_setters(tennerr, tennerrController,tennerrEscrow,tennerrFactory,tennerrVotingRightsToken,tennerrDAO, accounts):
    ...
    tennerrController.setTennerr(tennerr,{'from': accounts[0]})
...
def testRedeem(tennerr, tennerrEscrow,tennerrDAO, tennerrFactory, accounts, usdc, interface ):
    # tuple
    ...
    assert interface.IERC20(tennerrFactory).balanceOf(tennerrEscrow) == 0;
    assert interface.IERC20(tennerrFactory).balanceOf(accounts[2]) > 0;
    balance = interface.IERC20(tennerrFactory).balanceOf(accounts[2])
    usdcBalPre = usdc.balanceOf(accounts[2])
    tennerr.redeemCredit({'from': accounts[2]})
    balance2 = interface.IERC20(tennerrFactory).balanceOf(accounts[2])
    usdcBalPost = usdc.balanceOf(accounts[2])
    assert usdcBalPost > usdcBalPre
