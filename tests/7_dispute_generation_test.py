...
def testDisputeWork(tennerr, accounts,tennerrDAO, usdc):
    # tuple
    ...
    success = tennerr.openDispute( quoteId, 1, {'from':accounts[2]}).return_value
    assert success == True

def testDisputeElegibility(tennerr, tennerrEscrow,tennerrDAO, accounts, usdc):
    # tuple
    quotes = tennerr.getQuotesByAddress(accounts[0],{'from': accounts[0]})
    # array
    quote = quotes[0]
    ...
    success = tennerr.openDispute( quoteId, 1, {'from':accounts[2]}).return_value
    # x = tennerrDAO.disputeJob(quoteId,accounts[2],accounts[0], 1 )
    elegibles = tennerrDAO.getElegibleList()
    print(elegibles)
